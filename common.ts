...
import { readableStreamToString, GoDaddyError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new GoDaddyError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new GoDaddyError(e, e.response?.data)
                }
                if (innerError instanceof GoDaddyError) {
                    // Got "GoDaddyError" from the above try block
                    throw innerError;
                }
